utils::globalVariables(c('browseURL','.activeDataset'))
################################################################################
################################# FONCTION DEFMACRO ############################
...
optionsframe <- tkframe(top,borderwidth=2,relief="ridge")
    ...
    nbcol<-tkentry(optionsframe,width=4,textvariable=col)    ### Nouvelle boite de texte
    row<-tclVar("2")
...
#        if (!is.null(variablefact)) tkgrid(quali.sup.lab,quali.sup.check,sticky="w")
  ...
  #! fonction associer au bouton OK, execute et detruit l'interface graphique
onOK <- function(){
  done = OnAppliquer()
  tkdestroy(top)
}
...
#    varActives <- varActives[!(varActives%in%variablefact)]
    ...
    ncp.boot<-as.numeric(tclvalue(ncp.boot.val))
...
#    }
    ...
    # gestion de la commande realisant la fonction Sorting Task
      commande.FAST<-paste(nom.res, '<-fast(', activeDataSet(), ', ncp=', ncp,', word.min=', mot,', sep.words="', separ,'", alpha=', alpha,', B=',B,', ncp.boot=',ncp.boot,', graph = FALSE)', sep="")
   ...
   #, col.ind="', Rcol.ind, '", col.ind.sup="', Rcol.ind.sup, '", col.quali.sup="', Rcol.quali, '", label=c("', paste(Rlabel, collapse='", "'), '"), invisible=c("quanti.sup","', paste(Rinvis, collapse='", "'), '")',
    ...
    if((Gchoix)&(length(ls(pat=nom.res))>0)){
      ...
      if (get(nom.res)$eig[1,2]!=100) {
        commande.plotE<-paste('boot(', activeDataSet(),', axes=c(', paste(Axe, collapse=", "), '), level.conf=', 1-alpha,',nbsim=',B,',ncp=',ncp.boot,')', sep="")
...
#  Iillu.funct(label=gettextRcmdr("Select supplementary individuals"), firstLabel=gettextRcmdr("Select supplementary individuals"))
 PLOT.MCA(label=gettextRcmdr("Graphical options"), firstLabel=gettextRcmdr("Graphical options"))
  ...
  ncp.boot.lab<-tklabel(OptionFrame,text=gettextRcmdr("Number of dimensions to build ellipses: "))
  ncp.boot.val<-tclVar("2")
  ncp.boot<-tkentry(OptionFrame,width=5,textvariable=ncp.boot.val)
  ...
  tkgrid(B.lab, B)
    tkgrid(ncp.boot.lab, ncp.boot)
  tkgrid(resu.lab, resu)
  tkgrid(tklabel(OptionFrame,text="")) # Ligne de blanc
  tkgrid.configure(ncp.lab, resu.lab, Axe.label,mot.lab,separ.lab,alpha.lab,B.lab,ncp.boot.lab, column=1, columnspan=4, sticky="w")
  tkgrid.configure(ncp,mot,separ,alpha,B,ncp.boot, resu, column=6, columnspan=2, sticky="e")
